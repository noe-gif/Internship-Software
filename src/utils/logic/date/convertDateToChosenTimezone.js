...
const convertDatetimeRepresentationFromUTCToNewTimezone = (dateToConvert, timezone) => {
  const options = {
    hour12: false,
  };
  ...
  return new Date(dateToConvert).toLocaleString(LOCALES, options);
};

const splitDate = (utcDateString) => {
  ...
  const hours = date.toLocaleString(LOCALES, { hour: '2-digit', hour12: false }).padStart(2, '0');
...
const representUTCDateToChosenTimezoneAndFormat = (
  utcDateString,
  dateFormat,
  timezone,
) => {
  ...
  const dateWithCorrectTimezone = convertDatetimeRepresentationFromUTCToNewTimezone(utcDateString, timezone);
...
export default representUTCDateToChosenTimezoneAndFormat;
